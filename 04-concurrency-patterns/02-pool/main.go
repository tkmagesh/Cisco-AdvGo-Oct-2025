...
func main() {
	/*
		...
		When a resource is Acquire()d from the pool
			the pool will check if it has any idle resources
			if yes, return the resource from the pool
			else create a new resource (using the given factory) and return the resource
			Make sure that when a resource is acquired, it is not acquired again until it is released back to the pool
